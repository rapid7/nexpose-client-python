...
class TagAttribute(JSON, HasID):
    ...
    def as_json(self):
        json_dict = {}
        if self.id:
            json_dict["tag_attribute_id"] = self.id
...
class Tag(JSON, HasID):
    ...
    def Create(name, type, source_name=None):
        if source_name is None:
            source_name = DEFAULT_SOURCENAME
    ...
    def as_json(self):
        ...
        if self.attributes:
            json_dict["attributes"] = [attr.as_json() for attr in self.attributes]
        return json_dict
