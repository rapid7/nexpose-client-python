...
def DemonstrateBackupAPI():
	def GetDownloadCallback(filename, total_size):
		def DownloadCallback(already_downloaded_data, new_data):
			...
			print "\rDownload of {0} is {1:.2f}% complete".format(filename, as_percentage(downloaded_size, total_size)),
	...
	if session.CreateBackup(platform_independent=True, description='This a test...'):
		...
		original_backup_names = map(lambda backup: backup.name, original_backups)
		new_backup_names = map(lambda backup: backup.name, session.GetBackups())
...
def DemonstrateTagAPI():
	...
	for tag in session.GetTags():
		...
		config = tag.config
		if config is not None: assert isinstance(config, nexpose.TagConfiguration)
		print "  ID:", tag.id
...
def DemonstrateRoleAPI():
	...
	for details in imap(session.GetRoleDetails, session.GetRoleSummaries()):
		...
		if details.name == 'test' or True: # remove 'or True' for testing
			...
			for key, value in details.assetgroup_privileges.iteritems():
				print '  {0}:'.format(key), value
			print 'Global Privileges:'
			for key, value in details.global_privileges.iteritems():
				print '  {0}:'.format(key), value
			print 'Site Privileges:'
			for key, value in details.site_privileges.iteritems():
				print '  {0}:'.format(key), value
...
def DemonstrateUserAPI():
	...
	for user in session.GetUserSummaries():
		...
		print "    Asset Groups:", ', '.join(imap(str, config.accessible_assetgroups))
		print "    Site Count:", statistics.site_count
		print "    Sites:", ', '.join(imap(str, config.accessible_sites))
